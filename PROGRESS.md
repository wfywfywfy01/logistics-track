...
## 2026-09-22：查询兼容、采集闭环与 PDCA 私网修复

- `/api/track/{value}` 保持订单号优先，并兼容外层服务把国际运单号放在路径中的旧调用；显式 `?tracking=` 契约不变，仍返回全部 `matches[]`。
- 修复 FedEx 不存在提示被页面帮助文字误判为签收、强制配对后顶层与包裹运单不一致，以及多包裹顶层最新事件取错包裹。
- 查询响应增加最近官网抓取结果、最后成功快照是否过期及最新事件所属运单，明确区分“物流未动”“最近抓取失败”和“旧快照过期”。
- 官网批量步骤使用独立 7200 秒上限；通知由持久队列重试，不再因通知失败重复整批抓官网。生产默认数据过期 18 小时、停滞 48 小时，均可通过环境变量覆盖。
- Compose 默认使用 Secure Cookie，并将物流容器加入 `dealer-knowledge` 私有网络；管理端口仅绑定宿主机回环地址。
- 本地验证：`python -m pytest -q` 为 181 passed / 1 个 Windows 跳过的 Linux `SIGALRM` 测试；发布与线上验收待提交后执行。

## 2026-09-18：只读查询 API 与对外开通准备
...
- 通知统一进入持久任务；发送开始后的空回执、未知回执、超时或本地回执写入失败保持 unknown，不再自动重发。异常状态由同一通知事件发送警告消息。
- 换单归档旧绑定官网轨迹，清除新绑定的旧观测，立即重算订单状态；重复运单被拒绝。后台包裹修改和审计在同一数据库事务中提交。
- 定时任务占锁不再返回成功，错过精确分钟后可补跑；批量抓取汇总线程失败；停滞按每个包裹的官网最新扫描和各承运商阈值判断。
- Compose 使用生产同名数据卷和本机18080端口；备份覆盖活动/死信附件，缺失必要原件时失败关闭；恢复校验附件并重建路径。后台增加请求读超时、连接上限和请求长度校验。
- 日报分别展示状态、完整事件、预计送达覆盖，未结案总数不受500条预览上限影响；订单详情增加官网轨迹时间线。
- 复审修复：固定 Compose 外部卷名；备份证据从同一数据库快照选取并覆盖未结审核引用的原件；恢复先切附件、数据库最后切换，失败回滚并保留旧数据库；状态与通知任务原子提交；换单强制记录操作者和原因；日报分母包含未查询包裹。
- 二次复审修复：未结审核引用的已完成收件原件不会被定期清理；附件以同一份字节计算哈希并写入备份；非零通知退出只有结构化明确拒绝才重试，其余保持 unknown；多包裹在整单结果缺失时逐包裹生成失败待办。
- 备份逐个把原件暂存到磁盘后流式写入 ZIP，峰值内存不再随受保护附件总量增长。
- 提交：`fb02854`、`c296468`、`8b6e762`、`58b6fcf`、`e48fe06`、`0d60dfc`、`f05cf7b`。本地全量验证：166 passed、1个仅 Linux 运行的 `SIGALRM` 测试跳过；Python 编译、两个部署脚本语法、Compose配置和 diff 检查通过。尚未推送、构建候选镜像或切换生产；Docker管理面、HTTPS、异地备份、三家实票和72小时灰度按测试计划待验收。
- 2026-09-15 验收记录：隔离数据/通知/备份恢复/运营/后台安全/承运商测试全部通过；UPS 实票因本机无代理未收到 `GetStatus`，FedEx 提供号码被官网明确判定为未找到，未提供 DHL 有效样本。自动化门禁通过，但 Linux 硬截止、三家授权实票、服务器安全配置、异地恢复和72小时灰度仍未完成，不能签署生产上线。
- 代理验证：提供的 SOCKS5 出口访问 UPS、DHL、FedEx 页面均 HTTP 200；Chromium 不支持直接认证 SOCKS5，入口新增 Xray SOCKS5 上游配置并保持浏览器使用本地无认证端口。尚未在服务器启动 Xray 实测追踪接口。
- 最新提交 `95d51c9`：代理配置回归测试后全量为 167 passed、1个仅 Linux 运行的 `SIGALRM` 测试跳过；未保存代理账号密码，未推送或部署。
...
- PR #17 已合并为 `a1d06f8`，两个 Linux CI 测试任务均通过。生产已切换到镜像 manifest `sha256:30aa66e1e5aa7a70a72659826afdca8073462069f8378725cfec5c1bb5c8f13e`；容器 `healthy`、重启数 0，`/healthz` 与数据库 `integrity_check` 均为 `ok`，台账保持 144 单。
- 发布后增量回填检查 44 个包裹：UPS 29 个中官网成功 24 个、完整事件 6 个；DHL 12 个中官网成功 11 个、完整事件 5 个；3 个 FedEx 候选号均被官网明确判定为未找到。35 个成功结果全部应用、失败 0；台账共保存 11 份官方完整事件快照。只有页面状态而无官方事件数组的成功结果不记作完整历史。
- 切换前备份为 `/app/backups/logistics-backup-20260915-154530.zip`，旧容器保留为 `logistics-track-rollback-full-tracking-20260915`。FedEx 解析、持久化和回归测试已完成，但仍需一条官网可查的真实 FedEx 运单完成生产端到端验收。
